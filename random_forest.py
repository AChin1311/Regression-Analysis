...
for ndepths in num_depths:
  oob_errors = []
  for ntrees in num_trees:
    print("Doing OOB - depths: ", ndepths, ", trees: ", ntrees)
    # OOB (out of bag) error validation
    regr = RandomForestRegressor(n_estimators=ntrees, max_depth=ndepths, max_features= f, oob_score=True, n_jobs=-1)
    regr.fit(X, Y)
    oob_errors.append(1- regr.oob_score_)
...
plt.ylabel('OOB')
plt.legend()
plt.savefig('plot/2b(iii)-OOB.png')
plt.clf()
...
for ndepths in num_depths:
  ...
  for ntrees in num_trees:  
    ...
    for train_index, test_index in kf.split(X):
      ...
      train_size = X_train.shape[0]
      test_size = X_test.shape[0]
      regr = RandomForestRegressor(n_estimators=ntrees, max_depth=ndepths, max_features= f, n_jobs=-1)
      regr.fit(X_train, Y_train)
      Y_test_predict = regr.predict(X_test)
      test_mse.append(mean_squared_error(Y_test, Y_test_predict))
    rmse.append(np.sqrt(np.mean(test_mse)))
...
plt.ylabel('RMSE')
plt.legend()
plt.savefig('plot/2b(iii)-RMSE.png')
plt.clf()
